...
class Crawler():
    ...
    def get_songs_index_toplist(self, toplist_id):
        '''进入所选歌单页面，得出歌单里每首歌各自的ID 形式就是“song?id=64006"'''
        url=self.main_url+'discover/toplist?id=%d'% toplist_id
        re= self.session.get(url)   #直接用session进入网页，懒得构造了
        sel=Selector(text=re.text)   #用scrapy的Selector，懒得用BS4了
        song_ids=sel.xpath('//ul[@class="f-hide"]/li/a/@href').extract()
        song_names = sel.xpath('//ul[@class="f-hide"]/li/a/text()').extract()
        song_list=[]
        for index,id in enumerate(song_ids):   #enumerate表示可以从index变量中获取下标
            id=id.replace('/song?id=','')
            song=Song(song_id=id,song_name=song_names[index],song_num=1)
            song_list.append(song)

        return song_list   #所有歌曲组成的list

    def get_songs_index_playlist(self,playlist):
        '''进入所选歌单页面，得出歌单里每首歌各自的ID 形式就是“song?id=64006"'''
...
class Netease():
    ...
    def download_song_by_toplist_hot(self,number=200):
        songs=self.crawler.get_songs_index_toplist(3778678)         #热歌榜的id就是3778678
        dir_path=r'music'
        for index,song in enumerate(songs):
            self.download_song_by_id(song.song_id,song.song_name,index,self.folder)
            print("已下载 %s, 当前进度:%d/%d %f %%"% (song.song_name,index,number,index/number*100))
            if index>number:
                break

    def download_song_by_playlist(self,list_id):
        songs=self.crawler.get_songs_index_playlist(list_id)         #热歌榜的id就是3778678
        dir_path=r'music'
        for index,song in enumerate(songs):
            self.download_song_by_id(song.song_id,song.song_name,index,self.folder)
            print("已下载 %s, 当前进度:%d"% (song.song_name,index))
